...
class BitPage(BasePage):
    ...
    def should_be_popup_time_window_with_specified_button(self, button_name):
        self.should_be_window_popup_timeman()
        self.should_be_specified_button_in_window_popup_timeman(button_name)
    ...
    def should_be_specified_popup_window_after(self, first_button_text, change_button_text):
        ...
        button = f'//span[text()="{change_button_text}"]'
    ...
    def should_be_user_avatar(self):
        assert self.is_element_present(*BitPageLocators.USER_AVATAR), \
            '\nГде-то мы обосрались:' \
            '\nЭлемент не был представлен на странице:' \
    ...
    def should_be_specified_button_in_user_window_popup(self, button_text):
        ...
        assert self.is_element_present(how, what + button), \
            '\nГде-то мы обосрались:' \
            '\nЭлемент не был представлен на странице:' \
            f'\nКнопка "{button_text}" в попап-окне пользователя'
    ...
    def select_specified_button_in_window_popup_timeman(self, button_text):
        how, what = BitPageLocators.WINDOW_POPUP_TIMEMAN
        button = f'//span[text()="{button_text}"]'
        self.wait_for_present_and_click(how, what + button)
    ...
    def select_user_avatar(self):
        self.hover_mouse_to_element_and_click(*BitPageLocators.USER_AVATAR)
    ...
    def should_be_window_popup_avatar_header(self):
        assert self.is_element_present(*BitPageLocators.WINDOW_POPUP_AVATAR_HEADER), \
            '\nГде-то мы обосрались:' \
            '\nЭлемент не был представлен на странице:' \
            f'\nПопап-окно с статусами пользователя'

    def should_be_window_popup_timeman(self):
        assert self.is_element_present(*BitPageLocators.WINDOW_POPUP_TIMEMAN), \
            '\nГде-то мы обосрались:' \
            '\nЭлемент не был представлен на странице:' \
            f'\nПопап-окно с статусами пользователя'

    def should_be_specified_button_in_window_popup_timeman(self, button_name):
        how, what = BitPageLocators.WINDOW_POPUP_TIMEMAN
        button = f'//span[text()="{button_name}"]'
        assert self.is_element_present(how, what + button), \
            '\nГде-то мы обосрались:' \
            '\nЭлемент не был представлен на странице:' \
            f'\nВ попап-окне со временем не представлена кнопка "{button_name}"'
